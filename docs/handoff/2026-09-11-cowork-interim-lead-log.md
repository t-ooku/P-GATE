...
| 調査・実装（本 commit、大隆さん 9/20 13:15 報告「韓国 頭皮ケア リリーブ で Google 枠が出ない」） | 本番の `google_mall_search_usage_daily` は当該時刻に予約 +1（Google へは行っている）。何が返り何で落ちたかが本番で見えないため、1 検索 1 行のログ `google_mall_search_log`（migration 0083、本番適用済み: 検索語 120 字・source・reason・Google 返却件数・商品ページ件数・除外後件数・除外モール・所要 ms、14 日で purge）を追加。あわせて Agent Search の待ち時間 3 秒→7 秒（本検索と並行なので体感は変わらない。初回応答が 3 秒を超えて TIMEOUT になる疑い）。次: 大隆さんに再検索してもらい、ログの reason（TIMEOUT／HTTP_403／raw 0／全件除外）で原因を確定。 |
...
| 実装（本 commit、大隆さん 9/20 13:29 報告） | 通常の Google は「リリーブ」を「リリーイブ」に自動補正して商品を出す。Agent Search の検索要求に `spellCorrectionSpec:{mode:'AUTO'}` と `queryExpansionSpec:{condition:'AUTO'}` を明示（綴り違い・少件数でも同じ挙動に寄せる）。固定バーの「モールで直接探す」「ホシっとく」を同じ幅（2 列とも 1fr）。continuous-search.css v4。原因の最終確定は `google_mall_search_log` の次の 1 件で行う。 |
