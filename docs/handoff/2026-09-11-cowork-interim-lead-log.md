...
| 実装（本 commit、GPT 指示書 §P0/§6「90 日無反応の探し中は自動停止」） | `src/wish-idle-stop.mjs` を 15 分 cron の通常ジョブに追加。探し中（notify_new_match=1・insight_enabled_at あり・MUTED 以外）で、探し始めて 90 日以上・本人の操作（updated_at）も発見（search_watch_matches のベースライン以外）も 90 日無い条件に「まだ探し続けますか？」を 1 回だけ WEB 通知（event_type=INSIGHT_IDLE_CHECK）。通知から 14 日、本人の操作が無ければ notify_new_match=0・insight_enabled_at=NULL（＝あとで見る）。行は消さない。本番の探し中は最古で 9/8 開始のため、実際に動くのは 12 月以降。§54: 既存行の UPDATE は「指示書 P0 の自動停止」として承認済みの範囲、本人が触った条件は対象外。 |
| 原因確定・修正（本 commit、大隆さん 9/20 14:58 報告） | 本番ログ `google_mall_search_log`: 05 時台 `cache:RAW_0=4 / cache:SHOWN=1`。「韓国 頭皮ケア リリーブ」は Google へ行かず **空のキャッシュ** に当たっていた。綴り補正を入れる前（13:26 頃）の 0 件結果を 24 時間キャッシュしていたため、補正後も同じ検索語は空のまま。修正: 0 件の結果は 10 分だけキャッシュ（`EMPTY_CACHE_TTL_SECONDS`）、キャッシュ鍵を v2→v3 にして旧キャッシュを読まない。 |
