...
> **⚠️ 2026-09-19 更新: 本書の曜日別スケジュール（月水土=Runway 新規 Reel／火木日=カルーセル／金=既存素材編集 Reel／Stories 毎日）は `docs/handoff/2026-09-17-sns-policy-v3.md`（大隆さん 9/17 決定）で置き換え済み。**
> 正本は v3 のみ: 火・金 20:15 = Runway 新規 Reel（火=ユーザー向け、金=セラー向け）、月・水・土 20:00 = カルーセル、木・日 = 要求なし、適用開始 2026-09-21。
> 9/18〜9/20 は移行期間: v2 日次 Reel は大隆さん 9/16 承認で CANCELLED 済み、土曜（9/19・9/20）の Runway 新規 Reel は**要求しない**。9/18（金）のセラー向け Runway Reel は IG・X とも PUBLISHED（v3 の初回）。
> 監視（`check-social-ai-actress-sla.mjs`）と Codex の日次判定は v3 基準で行い、本書の曜日表で「未達」を立てない。#123 は本更新で解消。
