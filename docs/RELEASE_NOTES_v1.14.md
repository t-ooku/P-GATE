...
- 公開回答用購入先サニタイズ
- 複数ECの自動テストと公開前診断
- 既存Amazon URLから未承認購入先下書きを重複なく作成
- `Marketplace_Offer_Validation`で承認済みエラーと未完成下書きを分離
- 既存KPIを変更しないMarketplace別クリック・送客ログと選択シェア集計
- Marketplace購入先シートのプルダウン・数値制限・チェックボックス・入力説明
