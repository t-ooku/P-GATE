...
ローカル実装・自動テストまで完了。Google Apps Scriptへの反映待ち。

- 空の入力フォルダを5分ごとに確認しても、不要なSystem_Log・JSONログを増やさない。
- 1実行1ZIPに制限し、複数ZIPがある場合は古い順で次回トリガーへ繰り越す。
- 30分以上`STARTED`の取込を`STALE_EXECUTION_RECOVERED`として自動確定し、次回再処理可能にする。
