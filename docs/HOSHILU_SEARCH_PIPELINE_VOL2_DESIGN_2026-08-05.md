...
## 付記4：Vol.2 Stage2の一時保留について（2026-08-05 追記）

CTO指示書v3.0を受け、Vol.2 Stage2（`query-structurer.mjs`拡張以降の段階的移行）は一旦保留する。理由は、検索品質を「コードではなく教師データで育てる」という事業方針が本日確定し、その基盤（D1スキーマ・日次取り込みパイプライン）を先に整備しないと、Stage2以降の実装が教師データを考慮しない形で進んでしまい、後日大規模な作り直しが発生するため。詳細は [HOSHILU_TEACHER_DATASET_SPEC_v1.0.md](HOSHILU_TEACHER_DATASET_SPEC_v1.0.md) を参照。Stage 0〜1（本設計書§3-3〜3-4、付記2〜3の局所パッチ）は既に反映済みであり、これらは教師データ基盤の有無によらず有効。Vol.2 Stage2の再開は、教師データ基盤がどのように`query-structurer.mjs`へ接続されるかを設計し直してからとする。
