...
- 異常時は同名のGitHub Issueを1件だけ自動作成・更新する。最初の検出時刻より後の
  3回連続成功後にだけ復旧記録を追加して自動closeし、失敗Runの再実行が障害前の成功を
  数えて早期closeしないようにする。Issueを乱立させない。
...
- 5分監視はcomponent別の欠測・古さと失敗を確認する。楽天・Yahoo!は20分、Query Structurer・
  ...
  全componentの設定・認証・モデル・価格表・予算・usage異常は1回で検知し、Query Structurerと
  AIチャット主系を含むその他の一時失敗だけは異なる2回の連続失敗で
...
- Query Structurer、AIチャット主系、OpenAI予備系の通常`:07` slotが一時エラーで失敗した場合だけ、
  ...
  Query Structurer、AIチャット主系、OpenAI予備系は確認も失敗した時に異なる2回の連続失敗として
  Issue化する。設定・認証・モデル・価格・予算などの非一時エラーは
