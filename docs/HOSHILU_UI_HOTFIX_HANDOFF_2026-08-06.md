...
## 0-A. 現在の進捗（2026-08-06時点で確認済み・全項目未着手）

git履歴で確認した事実: `hotfix/ui-mall-links-order-2026-08-06`ブランチには、このドキュメント自体のコミット以外、コード変更は1つも入っていない。直近のコード変更コミットは`406f7d6`（§1参照）のまま。つまり以下は**全て未完了**。

- [ ] ① モール10個のカラーボタンが復帰している
- [ ] ① SNS5個のカラーボタンが復帰している
- [ ] ① 各ボタンが検索語付きリンクとして動作している（トップページ直行ではない）
- [ ] ② 添付画像の青文字リンクが完全に消えている（CSSで隠すだけの対応は不可）
- [ ] ② 青文字リンクが出ていた根本原因を特定済み
- [ ] ③④⑤ `MARKETPLACE COVERAGE`→`HOSHILU OFFICIAL`→`HOSHILU SEARCH AGENT`→`HOSHILU DISCOVERY`が連続ブロックになっている（現状は§2の通り、間に`HOSHILU NEWS`/`HOSHILU SALE RADAR`/`WHY HOSHILU`/`HOSHILU INSIGHT`が挟まったまま）
- [ ] PC表示確認
- [ ] スマートフォン表示確認
- [ ] 全テストPASS
- [ ] GitHubへpush
...
次にこのファイルを開くセッションは、まずこのチェックリストと本番の実際の見た目を突き合わせて、どこまで進んだかを更新してから作業を続けること。
