...
- Amazon、楽天市場、Yahoo!ショッピング、Qoo10、SHEINは全ジャンル共通の主要5モールとして維持する。
...
- 主要5モールにファッション検索限定の追加5モールを加え、最大10モールを対象に商品URL検証・フィード取込・7日鮮度診断・署名付き商品提示を行う。
- 商品候補は確認済みの商品詳細URLを1モール1件、最大10件まで署名付きで提示し、未確認リンクを収益リンクとして扱わない。
...
| 楽天市場 | 稼働中 | 楽天API・フィード対応 | `affiliateUrl` 優先処理を実装済み | 個人運営での利用条件と成果レポートを継続確認 |
| Yahoo!ショッピング | 稼働中 | 確認済み商品詳細URL | 承認・利用条件を確認できたリンクだけ採用 | 商品URLの検証と成果確認を継続 |
...
- HOSHILUの所有者・運営主体はオオクツタカノリ個人であり、アフィリエイト収入は本人の個人口座へ帰属する。
- Amazon.co.jpのwith care、Find fun、Tomorrow's smileはITGグループ株式会社が所有・運営する販売店舗であり、HOSHILUの所有主体と混同しない。
