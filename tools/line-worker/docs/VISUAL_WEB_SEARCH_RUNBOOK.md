...
Geminiが空応答を返しても、Cloud Vision側で完全一致または部分一致があり、
一致ページが2ホスト以上にまたがり、`bestGuessLabels`もある場合に限って、
そのサニタイズ済みラベルを信頼度0の検索仮説として使用する。単一ホスト、
類似画像だけ、Web entityだけでは救済しない。通常の画像単体解析ではJSON Schemaを
必須化し、商品名が不明でも中央の非人物オブジェクトを一般商品カテゴリと見た目の
属性に変換する。これらは追加の外部API呼び出しを行わない。
