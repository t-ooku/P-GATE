...
test('PixelからGalaxy Ultraへの逆訂正は新機種とUSB充電方式を保持する', () => {
  const queries = [
  ...
  for (const query of queries) {
    for (const marketplace of SEARCH_MARKETPLACES) {
      assert.equal(buildMarketplaceSearchKeywords(query, marketplace),
        'Galaxy S24 Ultra ケース USB充電式 光る', `${marketplace}: ${query}`);
    }
  }
});

test('音声入力風の端末訂正と電池不要の口語表現を4言語で正規化する', () => {
  const queries = [
...
test('音声認識で分割されたGalaxy型番も4言語で正規化して保持する', () => {
  const queries = [
...
test('強化ガラスを取り消した4言語検索はPET保護フィルムへ切り替える', () => {
  const queries = [
  ...
  for (const query of queries) {
    for (const marketplace of SEARCH_MARKETPLACES) {
      assert.equal(buildMarketplaceSearchKeywords(query, marketplace),
        'iPhone 16 Pro 保護フィルム PET', `${marketplace}: ${query}`);
    }
  }
});
...
test('指紋防止と飛散防止の複合仕様を4言語で同時に保持する', () => {
  const queries = [
  ...
  for (const query of queries) {
    for (const marketplace of SEARCH_MARKETPLACES) {
      assert.equal(buildMarketplaceSearchKeywords(query, marketplace),
        'iPhone 16 Pro 保護フィルム 指紋防止 飛散防止', `${marketplace}: ${query}`);
    }
  }
});
...
test('単焦点レンズは4言語のF値訂正で否定された旧F値を捨てる', () => {
  const queries = [
  ...
  for (const query of queries) {
    for (const marketplace of SEARCH_MARKETPLACES) {
      assert.equal(buildMarketplaceSearchKeywords(query, marketplace),
        'Sony Eマウント 50mm F1.4 単焦点レンズ', `${marketplace}: ${query}`);
    }
  }
});
...
test('ポータブルSSDは4言語の容量訂正で否定された旧容量を捨てる', () => {
  const queries = [
  ...
  for (const query of queries) {
    for (const marketplace of SEARCH_MARKETPLACES) {
      assert.equal(buildMarketplaceSearchKeywords(query, marketplace),
        'ポータブルSSD 1TB USB 3.2 Gen 2 読込 1050MB/s 耐衝撃', `${marketplace}: ${query}`);
    }
  }
});
...
test('ポータブルSSDは4言語のUSB世代訂正で省略された新世代を保持する', () => {
  const queries = [
...
test('イヤホンからヘッドホンへの言い直しは4言語で訂正後の商品種別だけを保持する', () => {
  const queries = [
  ...
  for (const query of queries) {
    for (const marketplace of SEARCH_MARKETPLACES) {
      assert.equal(buildMarketplaceSearchKeywords(query, marketplace),
        'ノイズキャンセリングヘッドホン オーバーイヤー Bluetooth 5.3', `${marketplace}: ${query}`);
    }
  }
});
...
test('smartwatch band size corrections discard the negated case size in four languages', () => {
  const queries = [
...
test('smartwatch model corrections inherit the omitted Apple Watch brand in four languages', () => {
  const queries = [
...
test('カメラ付き自動給餌器は容量と画質の同時訂正を4言語で反映する', () => {
  const inputs = [
