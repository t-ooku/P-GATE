...
test('discovery collage is lightweight, localized, accessible, and cached', async () => {
  ...
  assert.match(html, /カードの「価格比較」で最安を確認し/);
  ...
  assert.match(html, /id="journeyTitle"><span>4つの探し方から、<\/span><span>購入先・価格通知まで。<\/span>/);
  ...
  assert.match(html, /HOSHILUが2モールをまとめて比較し、その他11を含む合計最大13モールで探せます。/);
