...
import { amazonStorefrontUrl, handlePublicShopDirectoryRoute } from '../src/seller-shop.mjs';

test('GET /api/shops は掲載中ショップを slug/name/logo/tagline/coupon/url だけで返す（seller_key は出さない）', async () => {
  const rows = [
    { seller_key: 'k1', slug: 'with-care', shop_name: 'with care', logo_url: 'https://hoshilu.app/shop-logos/with-care.svg', tagline: '思いやりのある暮らし', tenants: '["itg"]', seller_ids: '["A19ONFBH56J9DF"]', live_coupons: 0 },
  ...
  // 2026-09-05 夜: 各店舗の Amazon ストアフロント（出品者IDから）
  assert.equal(payload.shops[0].amazon_url, 'https://www.amazon.co.jp/s?me=A19ONFBH56J9DF&tag=hoshilu00-22');
  assert.equal(payload.shops[1].amazon_url, '');
  assert.equal(amazonStorefrontUrl(['A3NNU8MHK7TN8Z'], 'hoshilu00-22'), 'https://www.amazon.co.jp/s?me=A3NNU8MHK7TN8Z&tag=hoshilu00-22');
  assert.equal(amazonStorefrontUrl(['not-an-id']), '');
