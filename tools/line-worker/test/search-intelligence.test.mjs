...
test('光るスマホケースは初回提示から一般ケース・LED照明・カテゴリ不明商品を除外する', () => {
  const queries = [
    ...
    'TikTokで見たメッセージが来るとスマホの背面が光るやつ',
    'the TikTok thing that makes the back of my phone light up when I get a message',
    '抖音上那个收到消息手机背面会亮的东西',
    '틱톡에서 본 메시지 오면 휴대폰 뒷면에 불 들어오는 거',
  ];
  const candidates = [
...
test('NFCで光る電池不要ケースは充電式・電池式・NFCタグ単体を除外する', () => {
  const queries = [
  ...
  const candidates = [
    { asin: 'MATCH', product_name: 'Battery-free NFC Light-up LED Smartphone Case no battery required' },
    { asin: 'CHARGE', product_name: 'Rechargeable NFC Light-up LED Smartphone Case' },
    { asin: 'BATTERY', product_name: 'Battery Powered NFC Light-up LED Smartphone Case' },
    { asin: 'NONFC', product_name: 'Battery-free Light-up LED Smartphone Case no battery required' },
    { asin: 'TAG', product_name: 'Battery-free NFC LED Tag Sticker for Smartphone' },
    { asin: 'PLAIN', product_name: 'NFC Smartphone Case Cover' },
  ];
  for (const query of queries) {
    assert.deepEqual(filterCategoryMismatches(query, candidates).map((candidate) => candidate.asin),
      ['MATCH'], query);
...
test('USB充電式からNFC電池不要への逆訂正は指定機種の新方式だけを提示する', () => {
  const queries = [
  ...
  const candidates = [
    { asin: 'MATCH', product_name: 'iPhone 15 Pro Battery-free NFC Light-up LED Case no battery required' },
    { asin: 'OLD', product_name: 'iPhone 15 Pro USB Rechargeable Light-up LED Case' },
    { asin: 'WRONGMODEL', product_name: 'iPhone 15 Pro Max Battery-free NFC Light-up LED Case no battery required' },
    { asin: 'NONFC', product_name: 'iPhone 15 Pro Battery-free Light-up LED Case no battery required' },
    { asin: 'PLAIN', product_name: 'iPhone 15 Pro NFC Phone Case Cover' },
    { asin: 'TAG', product_name: 'iPhone 15 Pro Battery-free NFC LED Tag Sticker' },
  ];
  for (const query of queries) {
    assert.deepEqual(filterCategoryMismatches(query, candidates).map((candidate) => candidate.asin),
      ['MATCH'], query);
...
test('GalaxyからPixelへの機種・方式同時訂正は新機種のNFC電池不要ケースだけを提示する', () => {
  const queries = [
  ...
  const candidates = [
    { asin: 'MATCH', product_name: 'Pixel 9 Pro Battery-free NFC Light-up LED Case no battery required' },
    { asin: 'OLD', product_name: 'Galaxy S24 Ultra USB Rechargeable Light-up LED Case' },
    { asin: 'WRONGMODEL', product_name: 'Pixel 9 Pro XL Battery-free NFC Light-up LED Case no battery required' },
    { asin: 'RECHARGE', product_name: 'Pixel 9 Pro USB Rechargeable Light-up LED Case' },
    { asin: 'NONFC', product_name: 'Pixel 9 Pro Battery-free Light-up LED Case no battery required' },
    { asin: 'PLAIN', product_name: 'Pixel 9 Pro NFC Phone Case Cover' },
  ];
  for (const query of queries) {
    assert.deepEqual(filterCategoryMismatches(query, candidates).map((candidate) => candidate.asin),
      ['MATCH'], query);
...
test('音声入力風の表記揺れでも訂正後PixelのNFC電池不要ケースだけを提示する', () => {
  const queries = [
    'ギャラクシー S24 ウルトラじゃなくてピクセル 9 プロのNFCで電池いらない光るケース',
    'no wait not Galaxy S24 Ultra I mean Pixel 9 Pro battery free NFC light up case',
    '不是Galaxy S24 Ultra我要Pixel 9 Pro不用装电池的NFC发光手机壳',
    '갤럭시 S24 울트라 아니고 픽셀 9 프로 배터리 필요 없는 NFC 발광 케이스',
  ];
  const candidates = [
    { asin: 'MATCH', product_name: 'Pixel 9 Pro Battery-free NFC Light-up LED Case no battery required' },
    { asin: 'OLD', product_name: 'Galaxy S24 Ultra Battery-free NFC Light-up LED Case no battery required' },
    { asin: 'XL', product_name: 'Pixel 9 Pro XL Battery-free NFC Light-up LED Case no battery required' },
    { asin: 'NONFC', product_name: 'Pixel 9 Pro Battery-free Light-up LED Case no battery required' },
    { asin: 'RECHARGE', product_name: 'Pixel 9 Pro USB Rechargeable Light-up LED Case' },
    { asin: 'PLAIN', product_name: 'Pixel 9 Pro NFC Phone Case Cover' },
  ];
...
test('単焦点レンズは4言語のF値訂正後の商品だけを提示する', () => {
  const queries = [
    'Sony Eマウント 50mm F1.8ではなくF1.4 単焦点レンズ',
    'Sony E-mount 50mm prime lens not F1.8 but F1.4',
    '索尼E卡口50mm定焦镜头不要F1.8，要F1.4',
    'Sony E마운트 50mm 단렌즈 F1.8 말고 F1.4',
  ];
  const candidates = [
    { asin: 'MATCH', product_name: 'Sony E-mount 50mm F1.4 Prime Lens' },
    { asin: 'OLD', product_name: 'Sony E-mount 50mm F1.8 Prime Lens' },
    { asin: '35MM', product_name: 'Sony E-mount 35mm F1.4 Prime Lens' },
    { asin: 'RF', product_name: 'Canon RF-mount 50mm F1.4 Prime Lens' },
  ];
  for (const query of queries) {
    assert.deepEqual(filterCategoryMismatches(query, candidates).map((candidate) => candidate.asin), ['MATCH'], query);
  }
...
test('ポータブルSSDは4言語のUSB世代訂正後の商品だけを提示する', () => {
  const queries = [
    'ポータブルSSD 1TB USB 3.2 Gen1ではなくGen2 読込2000MB/s 耐衝撃',
    'portable SSD 1TB USB 3.2 Gen 1 not Gen 1 but Gen 2 2000MB/s shockproof',
    '便携式SSD 1TB USB 3.2 Gen1不要，改成Gen2 读取2000MB/s 抗震',
    '휴대용 SSD 1TB USB 3.2 Gen1 말고 Gen2 읽기 2000MB/s 충격방지',
  ];
  const candidates = [
    { asin: 'MATCH', product_name: 'Portable SSD 1TB USB 3.2 Gen2 Read 2000MB/s Shockproof' },
    { asin: 'OLD', product_name: 'Portable SSD 1TB USB 3.2 Gen1 Read 2000MB/s Shockproof' },
    { asin: 'SLOW', product_name: 'Portable SSD 1TB USB 3.2 Gen2 Read 1050MB/s Shockproof' },
    { asin: 'HDD', product_name: 'Portable HDD 1TB USB 3.2 Gen2 Read 2000MB/s Shockproof' },
  ];
  for (const query of queries) {
    assert.deepEqual(filterCategoryMismatches(query, candidates).map((candidate) => candidate.asin), ['MATCH'], query);
...
test('smartwatch band material corrections exclude the negated material in four languages', () => {
  const queries = [
  ...
  const candidates = [
    { asin: 'MATCH', product_name: 'Apple Watch Series 9 Band 45mm Silicone' },
    { asin: 'OLD', product_name: 'Apple Watch Series 9 Band 45mm Leather' },
    { asin: 'SIZE', product_name: 'Apple Watch Series 9 Band 41mm Silicone' },
    { asin: 'S8', product_name: 'Apple Watch Series 8 Band 45mm Silicone' },
  ];
  for (const query of queries) {
    assert.deepEqual(filterCategoryMismatches(query, candidates).map((candidate) => candidate.asin), ['MATCH'], query);
...
test('smartwatch model corrections exclude the negated generation in four languages', () => {
  const queries = [
    'Apple Watch Series 8ではなくSeries 9 45mm シリコンバンド',
    'Apple Watch Series 8, not Series 8 but Series 9, 45mm silicone band',
    'Apple Watch Series 8不要，改成Series 9，45mm硅胶表带',
    'Apple Watch Series 8 말고 Series 9 45mm 실리콘 밴드',
  ];
  const candidates = [
    { asin: 'MATCH', product_name: 'Apple Watch Series 9 Band 45mm Silicone' },
    { asin: 'OLD', product_name: 'Apple Watch Series 8 Band 45mm Silicone' },
    { asin: 'SIZE', product_name: 'Apple Watch Series 9 Band 41mm Silicone' },
    { asin: 'LEATHER', product_name: 'Apple Watch Series 9 Band 45mm Leather' },
  ];
  for (const query of queries) {
    assert.deepEqual(filterCategoryMismatches(query, candidates).map((candidate) => candidate.asin), ['MATCH'], query);
  }
...
test('カメラ付き自動給餌器の容量訂正は旧容量と画質違いを除外して訂正後の本体だけを提示する', () => {
  const queries = [
  ...
  const candidates = [
    ...
    { asin: 'OLD', product_name: 'automatic pet feeder 6L 2K camera Wi-Fi two-way audio' },
    { asin: 'LOWRES', product_name: 'automatic pet feeder 4L 1080p camera Wi-Fi two-way audio' },
  ];
  for (const query of queries) {
    assert.deepEqual(filterCategoryMismatches(query, candidates).map((item) => item.asin), ['MATCH'], query);
  }
