...
test('「いまの価格を見る」は「探す」タブを開いて検索を実行する', () => {
  ...
  assert.match(read('index.html'), /app\.js\?v=172/);
});
...
test('「探す」ではページ名の帯を出さず、検索候補に種別バッジが無く、モール導線は商品の下', () => {
  ...
  const html = read('index.html');
  for (const asset of ['tab-nav.css?v=3', 'tab-nav.mjs?v=5', 'search-suggest.mjs?v=3', 'app.js?v=172']) assert.ok(html.includes(asset), asset);
});
...
test('検索枠に「検索方法」の見出しが無く、フォームが枠の先頭にある', () => {
  ...
  assert.ok(html.includes('app.js?v=172'));
});
...
test('商品画像はカード 300px・拡大 600px を画像サーバーに要求する', () => {
  ...
  const html = read('index.html');
  assert.ok(html.includes('app.js?v=172') && html.includes('buzz-home.mjs?v=7'));
});
...
test('「値下がり待ち」の各行に「やめる」があり、押すと wish を削除して描き直す', () => {
  ...
  const html = read('index.html');
  assert.ok(html.includes('app.js?v=172') && html.includes('mywatch.css?v=4'));
});
