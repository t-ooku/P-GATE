...
test('AIチャットのmodule scriptは直前のapp.jsタグに吸収されず独立して読み込まれる', async () => {
  const html = await read('index.html');
  assert.match(html, /<script type="module" src="\/app\.js\?v=94"><\/script><script type="module" src="\/ai-search-ui\.mjs"><\/script>/);
  assert.doesNotMatch(html, /src="\/app\.js\?v=94"<\/script>/);
});
