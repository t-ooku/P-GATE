...
test('HOSHILU AI action stays onsite and marketplace buttons use accessible brand colors', async () => {
  ...
  assert.match(worker, /hoshilu-shell-v336/);
...
test('AIチャットのmodule scriptは直前のapp.jsタグに吸収されず独立して読み込まれる', async () => {
  const html = await read('index.html');
  assert.match(html, /<script type="module" src="\/app\.js\?v=97"><\/script><script type="module" src="\/ai-search-ui\.mjs"><\/script>/);
  assert.doesNotMatch(html, /src="\/app\.js\?v=97"<\/script>/);
});
