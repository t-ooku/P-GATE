...
test('HOSHILU AI action stays onsite and marketplace buttons use accessible brand colors', async () => {
  ...
  assert.match(worker, /hoshilu-shell-v408/);
...
test('AIチャットのmodule scriptは直前のapp.jsタグに吸収されず、修正版URLで独立して読み込まれる', async () => {
  const html = await read('index.html');
  assert.match(html, /<script type="module" src="\/assets-v147\/app\.js\?v=169"><\/script><script type="module" src="\/ai-search-ui\.mjs\?v=16"><\/script>/);
...
test('検索方法はスライド式2モードで、AI確認は3回目のNO後に他モール導線へ切り替わる',async()=>{
  ...
  assert.match(html,/<button id="askAiButton" class="primary ask-ai-button" type="button">AIで探す<\/button><button id="submitButton" class="primary direct-search-button" type="submit"><span id="submitText">すぐ検索<\/span>/);
