...
const SHIFT_JIS_MARKETPLACES = new Set(['ZOZOTOWN_JP', 'ABCMART_JP']);
// マツキヨはsearch_keyword=UTF-8、ABC-MARTはkeyword=Shift_JISで
// 検索語を維持できることを2026-08-08に実レスポンスで再確認した。
const LANDING_PAGE_ONLY_MARKETPLACES = new Set();
...
// 2026-08-08 再検証: マツキヨはsearch_keyword=UTF-8、@cosmeはname=UTF-8、
// ABC-MARTはkeyword=Shift_JISが正しい。実語とダミー語の件数差、ならびに
// ABC-MARTのUTF-8/Shift_JIS応答差を確認したため、この形式を固定する。
test('2026-08-08: マツキヨ・@cosme SHOPPING・ABC-MARTは検証済みの検索形式で検索語を維持する', () => {
  ...
  // search_keyword= : 実語で48件、存在しない語で0件になることを確認済み。
  const matsukiyoUrl = new URL(byMarketplace.MATSUKIYO_JP);
  assert.equal(matsukiyoUrl.pathname, '/store/catalogsearch/result');
  assert.match(matsukiyoUrl.searchParams.get('search_keyword'), /ブラウス/);
  assert.equal(matsukiyoUrl.searchParams.has('q'), false);

  // ABC-MARTはShift_JIS。UTF-8では日本語が文字化けし、Shift_JISなら
  // 「スニーカー」で3,211件となることを実レスポンスで確認済み。
  assert.match(byMarketplace.ABCMART_JP, /^https:\/\/www\.abc-mart\.net\/shop\/goods\/search\.aspx\?keyword=/);
  assert.doesNotMatch(byMarketplace.ABCMART_JP, /%E7%99%BD|%E3%83%96%E3%83%A9%E3%82%A6%E3%82%B9/);
});
