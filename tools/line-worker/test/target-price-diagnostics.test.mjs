...
test('ウォッチ診断は読取専用・集計だけで欠落IDと最近の巡回を区別する', async () => {
  ...
  sqlite.exec(`CREATE TABLE member_wishes(watch_price,condition_snapshot);
    CREATE TABLE target_price_observations(reason,observed_at,matched DEFAULT 0,price_jpy);
    ...
    INSERT INTO target_price_observations(reason,observed_at) VALUES('ABOVE_TARGET',datetime('now')),('NO_MATCH',datetime('now')),('REACHED','2020-01-01');
  ...
  assert.deepEqual(result.observations_all.map(row => row.reason), ['ABOVE_TARGET','NO_MATCH','REACHED']);
