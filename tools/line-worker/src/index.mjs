...
import { handleSellerPriceOfferRoute } from './seller-price-offer.mjs';
...
function redactSearchPersonalData(query) {
  return String(query || '')
    ...
    // 電話番号は「単独の数字列」だけを落とす。JAN(4902175435297 など)の内部に
    // 0始まりの並びが含まれても識別子を壊さない(2026-09-02 実機: 写真のJANが
    // 「49」に削られ、無関係な商品が出た)。
    .replace(/(?<!\d)(?:\+?81[-\s]?)?0\d{1,4}[-\s]?\d{1,4}[-\s]?\d{3,4}(?!\d)/gu, ' ')
    .replace(/(?<![\d+])\+81[-\s]?\d{1,4}[-\s]?\d{1,4}[-\s]?\d{3,4}(?!\d)/gu, ' ')
    .replace(/\s+/gu, ' ')
    .trim()
    .slice(0, 200);
}
