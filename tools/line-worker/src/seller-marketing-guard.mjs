...
export function assertSellerMarketingCurrent(post = {}, env = {}) {
  ...
  if (!seller) return;
  if (/[3３](?:か|ヶ|カ|ケ|箇)月.{0,15}(?:無料|[0０]円)|無料.{0,15}[3３](?:か|ヶ|カ|ケ|箇)月/u.test(caption)) {
  ...
  if (media && !media.split(/[,\s"\[\]]+/u).filter(Boolean).every(url => url.includes('/social/carousel/seller30-20260927/'))) {
    throw new Error('SELLER_MARKETING_REVIEW_REQUIRED:MEDIA_OFFER_UNVERIFIED');
  }
  if (/30日.{0,10}無料/u.test(caption) && !recruitmentVerified(env) && !/準備中/u.test(caption)) {
