...
function airPurifierBodyConstraints(value) {
  const text = String(value || '').normalize('NFKC');
  return {
    purifier: /(?:空気清浄機|air\s*purifier|空气净化器|空氣淨化器|공기\s*청정기)/iu.test(text),
...
function cordlessStickVacuumConstraints(value) {
  const text = String(value || '').normalize('NFKC');
  return {
    stickVacuum: /(?:コードレス.{0,16}スティック掃除機|cordless\s*stick\s*vacuum|无线杆式吸尘器|無線桿式吸塵器|무선\s*스틱\s*청소기)/iu.test(text),
...
function iplHairRemovalConstraints(value) {
  ...
  return {
    device: /(?:IPL\s*光美容器|IPL\s*(?:hair\s*removal\s*)?device|IPL\s*脱毛仪|IPL\s*脫毛儀|IPL\s*제모기|(?:家|自宅).{0,16}(?:ムダ毛|脱毛).{0,12}(?:ケア|処理)|(?:remove|reduce).{0,12}(?:body\s*)?hair.{0,16}(?:at\s*home|home)|在家.{0,12}(?:脱毛|除毛)|집에서.{0,12}(?:제모|털\s*제거))/iu.test(text),
