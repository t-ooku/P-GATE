...
// atomic rollout path /assets-v139/app.js. Relative specifiers from the latter
// would incorrectly resolve to /assets-v139/*.mjs and stop the whole UI.
...
function clearInstantMarketplaceHandoff(){elements.instantMarketplace?.replaceChildren();elements.instantMarketplace?.classList.add('hidden');}
function renderInstantMarketplaceHandoff(query,hasSupplementalInput=false){
  ...
  elements.instantMarketplace.replaceChildren(textElement('p','instant-marketplace-status',message||'Marketplace search links are ready now.'),card);
...
async function runKnowledgeSearch(options={}){
  ...
  elements.status.className='status';elements.status.textContent=t.loading;elements.submit.disabled=true;elements.results.setAttribute('aria-busy','true');
  ...
  document.dispatchEvent(new CustomEvent('hoshilu:search-execution-started',{detail:{executionId,inputType}}));
  ...
  if(isIndependentSearchText(submittedQuery)){
    renderInstantMarketplaceHandoff(submittedQuery,hasSupplementalInput);
  ...
  try{
    ...
    rememberMemberSearch(elements.query.value);renderResults(result,lastRequestId);finishInstantMarketplaceHandoff(false);
    ...
    renderResults(fallback,lastRequestId);finishInstantMarketplaceHandoff(true);
  ...
  }finally{if(isCurrentRun()){elements.submit.disabled=false;elements.results.setAttribute('aria-busy','false');}}
}
