...
document.querySelector('#sellerDemandOfferForm')?.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    // 2026-09-21 大隆さん決定「クリック課金をやめる」: 残高・予算で登録を止めなくなったので、
    // その3つの断り文句（BALANCE_REQUIRED / BUDGET_OFF / BUDGET_CAP）も無くした。
...
// Demand Match（通知した需要・有効クリック・除外）。
// 2026-09-21 大隆さん決定でこのクリックの課金はやめた。料金は月額だけなので、
// 画面にも金額・予算上限・残高を出さない。出すのは数えられた件数だけ。
...
function renderDemandMatch(dm) {
  ...
  if (!kpi('valid')) return;
  // 数えられなかったときに 0 と書かない。サーバーが数を返した時だけ数字にする。
  const count = (value) => (Number.isFinite(Number(value)) ? String(Number(value)) : '計測不能');
  kpi('notified').textContent = count(dm.notified);
  kpi('valid').textContent = count(dm.valid_clicks);
  kpi('excluded').textContent = count(dm.excluded_clicks);
  showDemandMatchStatus('このクリックに料金はかかりません（HOSHILU Seller は月額のみ）。件数は今後の参考のために数えています。');
}
