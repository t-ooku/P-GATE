...
const COPY = {
  JA: { kicker: 'WEB検索から発見', title: 'Google検索から発見', note: 'Google 検索（Amazon・Qoo10・SHEIN・ZOZOTOWN など）の結果です。価格は各ページに書かれていた参考価格（検索時点）で、HOSHILU が API で確認した価格ではありません。', price: '参考価格・検索時点', open: '商品を見る', prev: '前へ', next: '次へ' },
  EN: { kicker: 'FOUND VIA WEB SEARCH', title: 'Found via Google Search', note: 'Google Search results (Amazon, Qoo10, SHEIN, ZOZOTOWN, etc.). Prices are as listed on each page at search time, not API-verified prices.', price: 'Listed price at search time', open: 'View', prev: 'Previous', next: 'Next' },
  ZH: { kicker: '网页搜索结果', title: 'Google 搜索结果', note: 'Google 搜索结果。价格为页面当时标注的参考价，不是 HOSHILU 通过 API 确认的价格。', price: '参考价・搜索时点', open: '查看', prev: '上一页', next: '下一页' },
  KO: { kicker: '웹 검색 결과', title: 'Google 검색 결과', note: 'Google 검색 결과입니다. 가격은 각 페이지에 표시된 참고 가격(검색 시점)이며 API 확인 가격이 아닙니다.', price: '참고 가격・검색 시점', open: '보기', prev: '이전', next: '다음' }
};
