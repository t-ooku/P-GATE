...
requestForm.addEventListener('submit',async event=>{event.preventDefault();if(!emailEnabled)return;track('registration_started',{content:'email'});status.textContent='認証コードを送信しています…';const response=await fetch('/api/member/email/request',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email:document.querySelector('#memberEmail').value})});if(!response.ok){status.textContent=response.status===429?'1分後にもう一度お試しください。':'コードを送信できませんでした。';return;}verifyForm.classList.remove('hidden');status.textContent='メールに届いた6桁コードを10分以内に入力してください。';});
verifyForm.addEventListener('submit',async event=>{event.preventDefault();status.textContent='確認しています…';const response=await fetch('/api/member/email/verify',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email:document.querySelector('#memberEmail').value,code:document.querySelector('#memberEmailCode').value})});if(!response.ok){status.textContent='コードが違うか、有効期限が切れています。';return;}track('registration_completed',{content:'email'});location.replace('/?member=logged-in');});
lineButton.addEventListener('click',()=>{if(lineButton.getAttribute('aria-disabled')!=='true')track('registration_started',{content:'line'});});
