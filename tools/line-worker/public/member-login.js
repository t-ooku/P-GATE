...
fetch('/api/config',{cache:'no-store'}).then(response=>response.json()).then(config=>{
  if(config.line_login_configured){lineButton.href=`/api/member/line/start?${new URLSearchParams({next:safeNext})}`;lineButton.removeAttribute('aria-disabled');lineButton.classList.remove('disabled');lineButton.textContent='LINEで無料会員登録';}else lineButton.textContent='LINE登録は準備中';
...
verifyForm.addEventListener('submit',async event=>{event.preventDefault();status.textContent='確認しています…';const response=await fetch('/api/member/email/verify',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email:document.querySelector('#memberEmail').value,code:document.querySelector('#memberEmailCode').value})});if(!response.ok){status.textContent='コードが違うか、有効期限が切れています。';return;}location.replace(safeNext);});
