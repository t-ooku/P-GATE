...
// atomic rollout path /assets-v145/app.js. Relative specifiers from the latter
// would incorrectly resolve to /assets-v145/*.mjs and stop the whole UI.
...
const searchInputCopy={
  JA:{cameraAction:'カメラで撮る',cameraRetake:'撮り直す',cameraPhoto:'撮影した写真',screenshotAction:'画像を選ぶ',socialAction:'投稿URLを追加',socialLabel:'Instagram・TikTok・Xなどの公開投稿URL',notice:'写真・画像・投稿URLはHOSHILUに保存しません。商品特定の補助にGoogle Cloud VisionのWeb画像照合とGoogle Gemini APIを使う場合があります。顔・住所・伝票などは写さないでください。',imageReady:'この画像を商品候補の抽出に使います',removeImage:'追加した画像を削除',missing:'一言・公開投稿URL・写真・画像のどれか1つを追加してください。',imageType:'JPEG・PNG・WebPの画像を選んでください。',imageLarge:'画像が大きすぎます。別の画像を選んでください。',urlUnsupported:'Instagram・TikTok・X・Threads・Facebook・Pinterestの公開投稿URLを入力してください。'},
  EN:{cameraAction:'Take photo',cameraRetake:'Retake',cameraPhoto:'Photo just taken',screenshotAction:'Choose image',socialAction:'Add post URL',socialLabel:'Public Instagram, TikTok, X, or other supported post URL',notice:'HOSHILU does not store photos, images, or post URLs. Google Cloud Vision web-image matching and Google Gemini API may help identify products. Keep faces, addresses, and shipping labels out of frame.',imageReady:'This image will be used to extract product clues',removeImage:'Remove added image',missing:'Add a phrase, public post URL, photo, or image.',imageType:'Choose a JPEG, PNG, or WebP image.',imageLarge:'That image is too large. Choose another image.',urlUnsupported:'Enter a public Instagram, TikTok, X, Threads, Facebook, or Pinterest post URL.'},
  ZH:{cameraAction:'拍照搜索',cameraRetake:'重新拍摄',cameraPhoto:'刚拍摄的照片',screenshotAction:'选择图片',socialAction:'添加帖子链接',socialLabel:'Instagram、TikTok、X 等公开帖子链接',notice:'HOSHILU 不会保存照片、图片或帖子链接。识别商品时可能使用 Google Cloud Vision 网页图片匹配和 Google Gemini API。请勿拍入人脸、地址或快递单。',imageReady:'此图片将用于提取商品线索',removeImage:'删除已添加的图片',missing:'请添加一句描述、公开帖子链接、照片或图片。',imageType:'请选择 JPEG、PNG 或 WebP 图片。',imageLarge:'图片过大，请选择其他图片。',urlUnsupported:'请输入 Instagram、TikTok、X、Threads、Facebook 或 Pinterest 的公开帖子链接。'},
  KO:{cameraAction:'카메라로 촬영',cameraRetake:'다시 촬영',cameraPhoto:'촬영한 사진',screenshotAction:'이미지 선택',socialAction:'게시물 URL 추가',socialLabel:'Instagram·TikTok·X 등 공개 게시물 URL',notice:'HOSHILU는 사진·이미지·게시물 URL을 저장하지 않습니다. 상품 식별 보조에 Google Cloud Vision 웹 이미지 매칭과 Google Gemini API를 사용할 수 있습니다. 얼굴·주소·송장은 촬영하지 마세요.',imageReady:'이 이미지를 상품 단서 추출에 사용합니다',removeImage:'추가한 이미지 삭제',missing:'한마디, 공개 게시물 URL, 사진 또는 이미지를 추가해 주세요.',imageType:'JPEG·PNG·WebP 이미지를 선택해 주세요.',imageLarge:'이미지가 너무 큽니다. 다른 이미지를 선택해 주세요.',urlUnsupported:'Instagram·TikTok·X·Threads·Facebook·Pinterest의 공개 게시물 URL을 입력해 주세요.'}
};
...
async function saveInsightWatch(query){const value=String(query||'').trim();if(!value)return false;setWishes([value,...getWishes().filter(item=>item!==value)]);renderWishes();if(memberSession)return Boolean(await persistInsightWatch(value,true,watchFrequencyFor(value)));return true;}
function wishSavedCopy(){if(memberSession)return selectedCopy().wishSaved;return{JA:'この端末に条件を保存しました（通知は無料会員登録後）',EN:'Saved on this device (sign up free to receive alerts)',ZH:'已保存到此设备（免费注册后可接收提醒）',KO:'이 기기에 저장했습니다 (무료 회원 가입 후 알림 가능)'}[elements.language.value]||'Saved on this device';}
...
const notificationUiCopy={JA:{title:'購入希望価格ウォッチ通知',refresh:'更新',login:'無料会員でログインすると通知を確認できます。',empty:'通知はまだありません。',read:'既読にする',dismiss:'非表示'},EN:{title:'Target price alerts',refresh:'Refresh',login:'Sign in as a free member to see alerts.',empty:'No notifications yet.',read:'Mark read',dismiss:'Dismiss'},ZH:{title:'目标价提醒',refresh:'更新',login:'登录免费会员后即可查看提醒。',empty:'暂无提醒。',read:'标为已读',dismiss:'隐藏'},KO:{title:'목표 가격 알림',refresh:'새로고침',login:'무료 회원으로 로그인하면 알림을 확인할 수 있습니다.',empty:'아직 알림이 없습니다.',read:'읽음',dismiss:'숨기기'}};
  ...
  try{
    ...
    for(let attempt=0;attempt<maxAttempts;attempt+=1){
      try{
        ...
        const retryableTurnstileFailure=/TURNSTILE_/u.test(code)&&code!=='TURNSTILE_UNSUPPORTED';
