...
function buildSmartWatchBandSearchKeywords(query) {
  ...
  const material = [
    ['チタン', /(?:チタン|titanium|钛(?:金属)?|鈦(?:金屬)?|티타늄)/giu],
    ['ステンレス', /(?:ステンレス|stainless(?:\s*steel)?|不锈钢|不鏽鋼|스테인리스)/giu],
    ['レザー', /(?:レザー|本革|革|leather|皮革|真皮|가죽)/giu],
    ['シリコン', /(?:シリコン|silicone|硅胶|矽膠|실리콘)/giu],
  ].flatMap(([label, pattern]) => [...normalized.matchAll(pattern)].map((match) => ({ label, match })))
...
function buildFrenchDoorRefrigeratorSearchKeywords(query) {
  const normalized = String(query || '').normalize('NFKC');
  if (!/(?:冷蔵庫|refrigerator|fridge|冰箱|냉장고|まとめ買い.{0,20}保存.{0,16}氷.{0,12}自動|store.{0,20}week.{0,20}groceries.{0,20}(?:make\s+ice|ice).{0,12}automatically|存.{0,16}一周.{0,16}(?:采购|採購).{0,16}(?:自动|自動)制冰|일주일.{0,16}장본.{0,20}보관.{0,16}얼음.{0,12}자동)/iu.test(normalized)) return '';
...
function buildLaserProjectorSearchKeywords(query) {
  const normalized = String(query || '').normalize('NFKC');
  if (!/(?:プロジェクター|projector|投影仪|投影機|프로젝터|壁いっぱい.{0,12}大画面.{0,12}映画|movies?.{0,16}filling.{0,16}whole\s*wall.{0,16}huge\s*screen|电影画面.{0,12}铺满.{0,12}整面墙|영화\s*화면.{0,12}벽\s*가득.{0,12}크게)/iu.test(normalized)) return '';
  const resolution = /\b4\s*k\b/iu.test(normalized) ? '4K' : '';
...
function buildFullFrameMirrorlessCameraSearchKeywords(query) {
  const normalized = String(query || '').normalize('NFKC');
  const camera = /(?:フルサイズ.{0,12}ミラーレス(?:カメラ)?|full[- ]?frame.{0,12}mirrorless\s*camera|全画幅.{0,12}无反相机|全片幅.{0,12}無反相機|풀프레임.{0,12}미러리스\s*카메라|暗い場所.{0,16}手ぶれ.{0,16}高画質.{0,12}動画.{0,12}撮|steady.{0,8}high[- ]quality\s*video.{0,16}low\s*light|暗光环境.{0,12}稳定.{0,12}高画质视频|어두운\s*곳.{0,12}흔들림\s*없이.{0,12}고화질\s*영상.{0,12}찍)/iu.test(normalized);
  if (!camera) return '';
...
function buildNasSearchKeywords(query) {
  const normalized = String(query || '').normalize('NFKC');
  if (!/(?:\bNAS\b|network\s*attached\s*storage|网络附加存储|網路附加儲存|네트워크\s*결합\s*스토리지|家族.{0,8}写真.{0,8}動画.{0,20}安全に保存.{0,20}複数端末.{0,16}高速|safe\s*place.{0,12}family\s*photos.{0,8}videos.{0,24}fast\s*access.{0,24}multiple\s*devices|全家照片视频.{0,16}集中安全保存.{0,20}多台设备.{0,16}高速访问|가족\s*사진.{0,8}영상.{0,20}안전하게\s*저장.{0,20}여러\s*기기.{0,16}빠르게)/iu.test(normalized)) return '';
...
function buildFoldingElectricBikeSearchKeywords(query) {
  const normalized = String(query || '').normalize('NFKC');
  const bike = /(?:折りたたみ電動アシスト自転車|folding\s*(?:electric\s*bike|e-bike)|折叠电动自行车|折疊電動自行車|접이식\s*전기\s*자전거|車に積め.{0,12}坂道.{0,12}楽に走り.{0,12}長距離移動|fits\s*in\s*the\s*car.{0,16}climbs\s*hills\s*easily.{0,16}travels\s*far|放进汽车后备箱.{0,12}轻松爬坡.{0,12}长距离骑行|차에\s*싣고.{0,12}언덕.{0,12}쉽게\s*오르며.{0,12}장거리\s*이동)/iu.test(normalized);
...
function buildPortablePowerStationSearchKeywords(query) {
  const normalized = String(query || '').normalize('NFKC');
  const station = /(?:ポータブル電源|portable\s*power\s*station|便携式储能电源|便攜式儲能電源|휴대용\s*파워뱅크|停電時.{0,12}冷蔵庫.{0,12}動かし.{0,16}ソーラー.{0,12}充電|refrigerator\s*running.{0,16}outages.{0,20}recharge\s*by\s*solar|停电时.{0,12}带动冰箱.{0,16}太阳能充电|정전\s*때.{0,12}냉장고.{0,12}돌리고.{0,16}태양광.{0,12}충전)/iu.test(normalized);
...
function buildCompressorDehumidifierSearchKeywords(query) {
  ...
  const dailyMatches = [...normalized.matchAll(/\b(\d{1,2}(?:\.\d)?)\s*L\s*(?:\/\s*(?:日|day)|per\s*day|每天|每日|\/\s*일)/giu)];
...
function buildCameraPetFeederSearchKeywords(query) {
  const normalized = String(query || '').normalize('NFKC');
  const feeder = /(?:ペット(?:用)?自動給餌器|自動給餌器|automatic\s*(?:pet\s*)?feeder|自动喂食器|自動餵食器|자동\s*급식기|留守中.{0,20}(?:猫|犬|ペット).{0,20}(?:自動.{0,8}(?:ご飯|餌)|(?:ご飯|餌).{0,8}自動)|留守中.{0,12}(?:猫|犬|ペット).{0,24}(?:決まった時間|時間を決め).{0,16}(?:ごはん|ご飯|餌).{0,24}(?:映像|見ながら).{0,16}話しかけ|feeds?.{0,12}(?:cat|dog|pet).{0,20}automatically.{0,24}(?:away|not\s*home)|schedule\s*meals?.{0,24}(?:see|watch).{0,12}(?:and\s*)?talk.{0,16}(?:cat|dog|pet).{0,20}away|出门时.{0,20}自动.{0,8}(?:给)?(?:猫|狗|宠物)喂食|出门时.{0,16}定时.{0,8}(?:给)?(?:猫|狗|宠物)喂食.{0,24}看着.{0,16}说话|外出時.{0,20}自動.{0,8}(?:給)?(?:貓|狗|寵物)餵食|집을\s*비울\s*때.{0,20}(?:고양이|강아지|반려동물).{0,20}자동으로.{0,8}밥|외출\s*중.{0,12}(?:고양이|강아지|반려동물).{0,20}정해진\s*시간.{0,16}밥.{0,20}보며.{0,12}말하고)/iu.test(normalized);
  const capacityMatches = [...normalized.matchAll(/\b(\d(?:\.\d)?)\s*L\b/giu)];
  const capacity = capacityMatches.filter((match) => {
    const before = normalized.slice(Math.max(0, match.index - 12), match.index);
    const after = normalized.slice(match.index + match[0].length, match.index + match[0].length + 12);
    return !/(?:not|不要)\s*$/iu.test(before)
  ...
  return ['ペット自動給餌器', `${capacity}L`, `${camera}カメラ`, 'Wi-Fi', '双方向音声'].join(' ');
}
...
function buildIplHairRemovalSearchKeywords(query) {
  ...
  const flashMatches = [...normalized.matchAll(/(\d{1,3})\s*(?:万\s*(?:回|発|发|次)?|만\s*회)|\b(\d{5,7})\s*flashes?\b/giu)];
  ...
  const flashes = flashMatch?.[1] ? String(Number(flashMatch[1]) * 10000) : flashMatch?.[2] || '';
  const cooling = /(?:冷却(?:機能)?|冷やし|cool(?:ing|\s*my\s*skin)|冰感冷却|冷感|냉각)/iu.test(normalized);
  const levelMatches = [...normalized.matchAll(/(\d{1,2})\s*(?:段階|levels?|档|檔|단계)/giu)];
...
export function buildDeviceAccessorySearchKeywords(query) {
  ...
  const protectorGlassPattern = /(?:強化ガラス|ガラスフィルム|tempered\s*glass|钢化玻璃|鋼化玻璃|강화유리)/iu;
  const protectorGlass = label === '保護フィルム'
    && protectorGlassPattern.test(normalized)
    && !isNegatedAttribute(normalized, protectorGlassPattern)
    ? '強化ガラス' : '';
  const protectorPet = label === '保護フィルム' && /\bpet\b/iu.test(normalized) ? 'PET' : '';
  const protectorAntiGlarePattern = /(?:反射防止|アンチグレア|anti[- ]?glare|matte|防眩光|防眩|저반사|무광)/iu;
  const protectorAntiGlare = label === '保護フィルム'
    && protectorAntiGlarePattern.test(normalized)
    && !isNegatedAttribute(normalized, protectorAntiGlarePattern)
    ? '反射防止' : '';
...
const GENERIC_PRODUCTS = [
  ...
  ['スマホケース', /(?:back|rear).{0,16}of\s*(?:(?:my|the)\s*)?phone.{0,24}(?:glow|lights?\s*up).{0,32}(?:notifications?|messages?)/iu],
  ['スマホケース', /(?:알림|전화|메시지).{0,20}(?:스마트폰|휴대폰).{0,12}뒷면.{0,12}불\s*들어오/iu],
  ['スマホケース', /(?:スマホケース|携帯(?:ケース|カバー)|(?:phone|mobile)\s*(?:case|cover)|手机壳|手機殼|手机保护壳|手機保護殼|휴대폰\s*(?:케이스|커버)|스마트폰\s*(?:케이스|커버)|スマホ.{0,20}(?:着信|通知|メッセージ).{0,20}(?:背面|裏).{0,12}(?:光|ピカ)|(?:着信|通知|メッセージ).{0,20}スマホ.{0,12}(?:背面|裏).{0,12}(?:光|ピカ)|phone.{0,20}(?:back|rear).{0,24}(?:glow|lights?\s*up).{0,32}(?:notifications?|messages?)|(?:notifications?|messages?).{0,32}(?:back|rear).{0,16}of\s*(?:the\s*)?phone.{0,16}(?:glow|lights?\s*up)|手机.{0,20}(?:通知|来电|消息).{0,20}背面.{0,12}(?:会亮|发光)|(?:通知|来电|消息).{0,20}手机.{0,12}背面.{0,12}(?:会亮|发光)|(?:스마트폰|휴대폰).{0,20}(?:알림|전화|메시지).{0,20}뒷면.{0,12}(?:불이\s*들어오|빛나)|(?:알림|전화|메시지).{0,20}(?:스마트폰|휴대폰).{0,12}뒷면.{0,12}(?:불이\s*들어오|빛나))/iu],
...
const GENERIC_ATTRIBUTES = [
  ...
  // 2026-09-03 検索品質カナリア: 「韓国リップ」「韓国コスメ ピンク リップ」で
  // 「韓国」が落ちて国内ブランドのリップが本命になった。原産・ブランド圏の
  // 「韓国」(韓国風とは別)は商品語として残す。
  ['韓国', /(?:韓国(?!っぽ|風|系|の|で|から)|韓流|k-?beauty|kビューティ|korean(?!\s*(?:style|look))|韩国(?!风|系)|한국(?!풍|\s*스타일))/iu],
...
function buildMarketplaceSearchKeywordsCore(query, marketplace = 'QOO10_JP') {
  ...
  if (products.includes('スマホケース')
    && /(?:光る|発光|ピカピカ|ピカッ|LED|ライトアップ|lights?\s*up|light[- ]?up|glow(?:ing)?|luminous|发光|發光|会亮|會亮|빛나는|발광|불빛\s*나는|불이\s*들어오)/iu.test(normalized)) {
