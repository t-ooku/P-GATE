...
async function runKnowledgeSearch(options={}){
  ...
  try{
    ...
    if(!isIndependentSearchText(submittedQuery)&&hasSupplementalInput){elements.status.className='status error';elements.status.textContent={JA:'写真・画像・投稿URLから候補を読み取れませんでした。色・形・用途などを一言だけ足して、もう一度お試しください。',EN:'We could not read a product clue from that photo, image, or post. Add a color, shape, or use and try again.',ZH:'无法从照片、图片或帖子中提取商品线索。请补充颜色、形状或用途后重试。',KO:'사진·이미지·게시물에서 상품 단서를 읽지 못했습니다. 색상·모양·용도를 추가해 다시 시도해 주세요.'}[elements.language.value]||'Could not analyze this input.';return{ok:false,error:failureTelemetry.error_code,requestId:failureTelemetry.request_id};}
