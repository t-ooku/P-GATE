...
function productCard(candidate,index,t,confirmed){
  const card=document.createElement('article');
  ...
  if(candidate.image){const image=document.createElement('img');image.className='product-image';image.src=candidate.image;image.alt='';image.loading='lazy';image.referrerPolicy='no-referrer';card.append(image);}
...
function renderResults(result,requestId){
  ...
  const rows=[
    resultRow(confirmed.map((candidate,index)=>productCard(candidate,index,t,true)),copy.confirmedTitle,copy.confirmedNote,'confirmed'),
    resultRow(unconfirmed.map((candidate,index)=>productCard(candidate,index,t,false)),copy.unconfirmedTitle,copy.unconfirmedNote,'unconfirmed')
  ].filter(Boolean);
  if(rows.length){
    const resultCards=[];
    ...
    resultCards.push(refinementCard());
    ...
    wish.addEventListener('click',()=>{saveWish(elements.query.value);wish.textContent=t.wishSaved;wish.disabled=true;});
    empty.append(wish);
