...
async function runKnowledgeSearch(options={}){
  ...
  try{
    ...
    if(failureTelemetry.error_code==='TURNSTILE_TOKEN_UNAVAILABLE'){elements.status.className='status error';elements.status.textContent={JA:'セキュリティ確認を完了して、もう一度「検索する」を押してください。',EN:'Complete the security check, then select Search again.',ZH:'请完成安全验证，然后再次点击“搜索”。',KO:'보안 확인을 완료한 뒤 다시 검색해 주세요.'}[elements.language.value]||'Complete the security check, then search again.';elements.turnstile?.scrollIntoView({behavior:'smooth',block:'center'});return{ok:false,error:failureTelemetry.error_code,requestId:failureTelemetry.request_id};}
