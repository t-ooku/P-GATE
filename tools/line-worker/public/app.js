...
function marketplaceFallbackGroup(label,linksNode,description){if(!linksNode)return null;const group=document.createElement('div');group.className='marketplace-fallback-group';group.append(textElement('p','marketplace-fallback-group-label',label));if(description)group.append(textElement('p','marketplace-fallback-group-description',description));group.append(linksNode);return group;}
// v4.2 項目17: integrated/directの判定はsrc/index.mjsのsearchModeForMarketplace
// が唯一の判定元で、/api/knowledge のレスポンスの各リンクにmodeとして載って
// くる。emergencyMarketplaceFallback()のオフライン用リンクだけはサーバーを
// 経由しないためmodeを持たず、ここで同じ3モールのSetを使って補う。
const CLIENT_INTEGRATED_MARKETPLACES=new Set(['AMAZON_JP','RAKUTEN_JP','YAHOO_JP']);
function linkSearchMode(item){return item?.mode||(CLIENT_INTEGRATED_MARKETPLACES.has(String(item?.marketplace||''))?'integrated':'direct');}
function marketplaceFallbackCard(result){if(!result?.marketplace_search_links?.length&&!result?.amazon_search_url)return null;const labels={
  JA:{heading:'13モールとSNSを横断して探す',integratedLabel:'まとめて検索',integratedBody:'HOSHILUが商品をまとめて探して比較します。',directLabel:'個別に探す',directBody:'HOSHILUの検索結果には含まれません。各ショップでも同じ条件で探せます。',socialLabel:'SNSで探す'},
  EN:{heading:'Search 13 marketplaces and social platforms',integratedLabel:'Search together',integratedBody:'HOSHILU searches and compares these marketplaces together.',directLabel:'Search individually',directBody:'Not included in HOSHILU’s results. You can search the same terms directly on each shop.',socialLabel:'Social search'},
  ZH:{heading:'跨13个商城和社交平台查找',integratedLabel:'一起搜索',integratedBody:'HOSHILU会整合搜索并比较这些商城。',directLabel:'单独搜索',directBody:'不包含在HOSHILU的结果中。您可以在各商城使用相同条件继续搜索。',socialLabel:'社交平台'},
  KO:{heading:'13개 쇼핑몰과 SNS에서 함께 찾기',integratedLabel:'한번에 검색',integratedBody:'HOSHILU가 상품을 모아 함께 비교합니다.',directLabel:'개별 검색',directBody:'HOSHILU 검색 결과에는 포함되지 않습니다. 각 쇼핑몰에서도 같은 조건으로 찾을 수 있습니다.',socialLabel:'SNS'}
};const {heading,integratedLabel,integratedBody,directLabel,directBody,socialLabel}=labels[elements.language.value]||labels.JA;const actions=actionCopy[elements.language.value]||actionCopy.JA;const language=elements.language.value;const card=document.createElement('article');card.className='refinement-card marketplace-fallback';card.id='marketplaceFallback';const intro=document.createElement('div');intro.className='marketplace-fallback-intro';intro.append(textElement('strong','',heading));const analysis=result?.ai_discovery?.analysis||null;const intentBlock=aiIntentSummaryBlock(analysis,language);if(intentBlock)intro.append(intentBlock);const candidateCards=aiCandidateCards(analysis?.product_candidates,language);if(candidateCards)intro.append(candidateCards);const keywords=String(elements.query.value||result?.search_keywords||result?.amazon_search_keywords||'');const tagSource=analysis?.search_keywords?.length?analysis.search_keywords.join(' '):keywords;const tagBlock=keywordTagBlock(tagSource,actions,(aiIntentLabels[language]||aiIntentLabels.JA).keywords);if(tagBlock)intro.append(tagBlock);card.append(intro);const legacy=result.amazon_search_url?[{marketplace:'AMAZON_JP',label:'Amazonで探す',url:result.amazon_search_url,mode:'integrated'}]:[];const allLinks=result.marketplace_search_links?.length?result.marketplace_search_links:legacy;const integratedItems=allLinks.filter(item=>linkSearchMode(item)==='integrated');const directItems=allLinks.filter(item=>linkSearchMode(item)==='direct');const integratedGroup=marketplaceFallbackGroup(integratedLabel,marketplaceLinks(integratedItems),integratedBody);if(integratedGroup)card.append(integratedGroup);const directGroup=marketplaceFallbackGroup(directLabel,marketplaceLinks(directItems),directBody);if(directGroup)card.append(directGroup);const socialLinks=marketplaceLinks(socialDiscoverySearchLinks(keywords));if(socialLinks){socialLinks.classList.add('social-search-links');const socialGroup=marketplaceFallbackGroup(socialLabel,socialLinks);if(socialGroup)card.append(socialGroup);}return card;}
