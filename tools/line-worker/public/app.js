...
Object.assign(copy.JA,{hero:'スクショ・投稿URL・一言から、|欲しい物を探せる。',title:'商品名が分からなくても大丈夫。\nスクショ、公開投稿URL、覚えている一言から探せます。',placeholder:'例：インスタで見た、ピンクで小さいカメラみたいなもの'});
Object.assign(copy.EN,{hero:'Find products from a |screenshot, post, or phrase.',title:'You do not need the product name.\nSearch from a screenshot, public post URL, or anything you remember.',placeholder:'Example: a small pink camera-like thing I saw on Instagram'});
Object.assign(copy.ZH,{hero:'从截图、帖子链接或一句描述，|寻找想要的商品。',title:'不知道商品名称也没关系。\n可通过截图、公开帖子链接或记得的一句话搜索。',placeholder:'例如：在 Instagram 看到的粉色小相机一样的东西'});
...
const searchInputCopy={
  JA:{screenshotAction:'スクショを追加',socialAction:'投稿URLを追加',socialLabel:'Instagram・TikTok・Xなどの公開投稿URL',notice:'スクショ・投稿URLはHOSHILUのサーバーに保存しません。候補抽出にはGoogle Gemini APIを使います。',imageReady:'商品候補の抽出に使います',removeImage:'スクリーンショットを削除',missing:'一言・公開投稿URL・スクショのどれか1つを追加してください。',imageType:'JPEG・PNG・WebPのスクショを選んでください。',imageLarge:'画像が大きすぎます。別のスクショを選んでください。',urlUnsupported:'Instagram・TikTok・X・Threads・Facebook・Pinterestの公開投稿URLを入力してください。'},
  EN:{screenshotAction:'Add screenshot',socialAction:'Add post URL',socialLabel:'Public Instagram, TikTok, X, or other supported post URL',notice:'HOSHILU does not store screenshots or post URLs on its servers. Google Gemini API extracts product clues.',imageReady:'Used only to extract product clues',removeImage:'Remove screenshot',missing:'Add a phrase, public post URL, or screenshot.',imageType:'Choose a JPEG, PNG, or WebP screenshot.',imageLarge:'That image is too large. Choose another screenshot.',urlUnsupported:'Enter a public Instagram, TikTok, X, Threads, Facebook, or Pinterest post URL.'},
...
function clearPreparedSearchImage(){searchImageGeneration+=1;preparedSearchImage=null;searchImagePreparing=false;if(elements.screenshot)elements.screenshot.value='';if(elements.screenshotPreviewImage)elements.screenshotPreviewImage.removeAttribute('src');elements.screenshotPreview?.classList.add('hidden');}
...
function setLanguage(language){
  ...
  if(elements.heroPromise)elements.heroPromise.textContent=inputLabels.heroPromise;if(elements.queryLabel)elements.queryLabel.textContent=inputLabels.queryLabel;if(elements.searchInputActions)elements.searchInputActions.setAttribute('aria-label',inputLabels.actionsLabel);if(elements.screenshotActionLabel)elements.screenshotActionLabel.textContent=inputLabels.screenshotAction;if(elements.socialUrlActionLabel)elements.socialUrlActionLabel.textContent=elements.socialUrlToggle?.getAttribute('aria-expanded')==='true'?inputLabels.socialActionClose:inputLabels.socialAction;if(elements.socialUrlLabel)elements.socialUrlLabel.textContent=inputLabels.socialLabel;if(elements.searchInputNotice)elements.searchInputNotice.textContent=inputLabels.notice;if(elements.screenshotPreviewStatus)elements.screenshotPreviewStatus.textContent=inputLabels.imageReady;if(elements.screenshotPreviewImage)elements.screenshotPreviewImage.alt=inputLabels.previewAlt;if(elements.removeScreenshot)elements.removeScreenshot.setAttribute('aria-label',inputLabels.removeImage);
...
elements.screenshot?.addEventListener('change',async()=>{
  const file=elements.screenshot.files?.[0];if(!file){clearPreparedSearchImage();return;}
  const generation=++searchImageGeneration;
  searchImagePreparing=true;preparedSearchImage=null;elements.screenshotPreview.classList.remove('hidden');elements.screenshotPreviewName.textContent=file.name;elements.screenshotPreviewStatus.textContent='…';elements.status.className='status';elements.status.textContent='';
  try{const prepared=await prepareSearchImage(file);if(generation!==searchImageGeneration)return;preparedSearchImage=prepared.payload;elements.screenshotPreviewImage.src=prepared.preview;elements.screenshotPreviewName.textContent=prepared.name;elements.screenshotPreviewStatus.textContent=selectedSearchInputCopy().imageReady;}
  catch(error){if(generation!==searchImageGeneration)return;clearPreparedSearchImage();elements.status.className='status error';elements.status.textContent=String(error?.message||selectedSearchInputCopy().imageType);}
  finally{if(generation===searchImageGeneration)searchImagePreparing=false;}
});
...
async function runKnowledgeSearch(options={}){
  ...
  const inputType=[submittedQuery?'TEXT':'',submittedImage?'SCREENSHOT':'',submittedSocialUrl?'SOCIAL_URL':''].filter(Boolean).join('_');
  ...
  try{
    ...
    if(!isIndependentSearchText(submittedQuery)&&hasSupplementalInput){elements.status.className='status error';elements.status.textContent={JA:'スクショ・投稿URLから候補を読み取れませんでした。色・形・用途などを一言だけ足して、もう一度お試しください。',EN:'We could not read a product clue from that screenshot or post. Add a color, shape, or use and try again.',ZH:'无法从截图或帖子中提取商品线索。请补充颜色、形状或用途后重试。',KO:'스크린샷이나 게시물에서 상품 단서를 읽지 못했습니다. 색상·모양·용도를 추가해 다시 시도해 주세요.'}[elements.language.value]||'Could not analyze this input.';return{ok:false,error:failureTelemetry.error_code,requestId:failureTelemetry.request_id};}
