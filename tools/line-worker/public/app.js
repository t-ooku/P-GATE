...
let turnstileWidget = null; let installPrompt = null; let memberSession = null; let memberWishRecords = []; let searchAttempt=0; let searchRoot=''; let rankingCategorySelection=null; let rankingConfirmationFlow=null; let rankingRequestSequence=0; const sessionId = getSessionId();
...
function createWatchOptions(candidate,t){
  ...
  const priceLabels={JA:{label:'この金額以下になったら購入したい',unit:'円',note:'API確認価格だけを監視します（AI推定価格は不使用）。希望額は5人以上で匿名集計し、契約セラーの需要分析にも使います。',login:'価格監視には無料会員ログインが必要です。'},EN:{label:'Notify me at or below this price',unit:'JPY',note:'We use API prices only, never AI estimates. Target prices may appear in seller demand analytics only in anonymous groups of 5+.',login:'Sign in as a free member to monitor a target price.'},ZH:{label:'降到此价格以下时通知',unit:'日元',note:'仅使用商城API确认价格。希望价仅在5人以上时匿名汇总，用于签约卖家的需求分析。',login:'价格监控需要登录免费会员。'},KO:{label:'이 가격 이하일 때 알림',unit:'엔',note:'쇼핑몰 API 확인 가격만 사용합니다. 희망가는 5명 이상일 때만 익명 집계해 계약 판매자 수요 분석에 활용합니다.',login:'목표 가격 감시에는 무료 회원 로그인이 필요합니다.'}}[elements.language.value]||null;
  const targetWrap=document.createElement('label');targetWrap.className='target-price-field';targetWrap.append(textElement('span','',priceLabels.label));
  const targetInput=document.createElement('input');targetInput.type='number';targetInput.inputMode='numeric';targetInput.min='100';targetInput.max='100000000';targetInput.step='100';const currentPrice=Math.min(...(Array.isArray(candidate?.offers)?candidate.offers:[]).map(offer=>Number(offer.total_cost||offer.price)||Infinity));if(Number.isFinite(currentPrice))targetInput.placeholder=String(Math.max(100,Math.floor(currentPrice*.9/100)*100));const productKey=String(candidate?.record_key||candidate?.asin||'');const productName=String(candidate?.display_name||candidate?.product_name||elements.query.value);const savedTarget=getWatchPreferences().find(item=>(productKey&&item.target_product_key===productKey)||item.target_product_name===productName);if(Number(savedTarget?.target_price_jpy)>=100)targetInput.value=String(savedTarget.target_price_jpy);targetWrap.append(targetInput,textElement('small','',priceLabels.unit));
