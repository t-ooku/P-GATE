...
async function renderTurnstileWidget(){if(!turnstileSiteKey||!elements.turnstile?.isConnected)throw new Error('TURNSTILE_CONTAINER_UNAVAILABLE');const api=await waitForTurnstileApi();turnstileUnsupported=false;try{turnstileWidget=api.render(elements.turnstile,{sitekey:turnstileSiteKey,theme:'light',size:'flexible',retry:'auto','retry-interval':3000,'refresh-expired':'auto','refresh-timeout':'auto',callback:onTurnstileToken,'expired-callback':clearTurnstileToken,'timeout-callback':clearTurnstileToken,'unsupported-callback':onTurnstileUnsupported,'error-callback':code=>{clearTurnstileToken();console.warn('TURNSTILE_CLIENT_ERROR',String(code||'').slice(0,40));return false;}});}catch(error){turnstileWidget=null;throw error;}}
