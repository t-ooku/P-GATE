...
function marketplaceFallbackCard(result){if(!result?.marketplace_search_links?.length&&!result?.amazon_search_url)return null;const labels={
  JA:{heading:'13モールとSNSを横断して探す',integratedLabel:'まとめて検索',integratedBody:'まとめて検索する場合は、ホシル検索へ。',directLabel:'個別に探す',directBody:'HOSHILUの検索結果には含まれません。各ショップでも同じ条件で探せます。',socialLabel:'SNSで探す'},
  EN:{heading:'Search 13 marketplaces and social platforms',integratedLabel:'Search together',integratedBody:'To search multiple stores at once, use HOSHILU Search.',directLabel:'Search individually',directBody:'Not included in HOSHILU’s results. You can search the same terms directly on each shop.',socialLabel:'Social search'},
  ZH:{heading:'跨13个商城和社交平台查找',integratedLabel:'一起搜索',integratedBody:'如需一次搜索多个商城，请使用HOSHILU搜索。',directLabel:'单独搜索',directBody:'不包含在HOSHILU的结果中。您可以在各商城使用相同条件继续搜索。',socialLabel:'社交平台'},
  KO:{heading:'13개 쇼핑몰과 SNS에서 함께 찾기',integratedLabel:'한번에 검색',integratedBody:'한번에 검색하려면 HOSHILU 검색을 이용하세요.',directLabel:'개별 검색',directBody:'HOSHILU 검색 결과에는 포함되지 않습니다. 각 쇼핑몰에서도 같은 조건으로 찾을 수 있습니다.',socialLabel:'SNS'}
