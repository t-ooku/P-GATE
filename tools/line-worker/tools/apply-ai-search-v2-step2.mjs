...
function env(){return{TURNSTILE_SECRET_KEY:'turnstile-secret',TURNSTILE_SITE_KEY:'site-key',GAS_BACKEND_URL:'https://gas.example.test/exec',GAS_BRIDGE_SECRET:'g'.repeat(32),LINK_SIGNING_SECRET:'l'.repeat(32),GEMINI_API_KEY:'x'.repeat(32),AMAZON_ASSOCIATE_TAG:'hoshilu00-22',PRODUCT_DB:{prepare(){return{bind(){return{all:async()=>({results:[]})}}}}}};}
function request(language){return new Request('https://hoshilu.app/api/knowledge',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({query:language==='KO'?'투명한 무선 이어폰':'韓国っぽい透明イヤホン',language,search_attempt:1,processing_notice_shown:true,session_id:'anonymous_session_123456',turnstile_token:'verified-token'})});}
