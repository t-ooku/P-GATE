...
var Config = (function () {
  ...
  var CURRENT_SYSTEM_VERSION = '1.6.0';
...
var LineIntegration = (function () {
  ...
  function findConfiguredContract() {
    var contractId = getRequiredProperty(PROPERTY_CONTRACT_ID);
    var sheets = ContractPolicyEngine.ensureSheets();
    var contracts = ContractPolicyEngine.loadContracts(sheets.contracts);
    for (var i = 0; i < contracts.length; i += 1) {
  ...
  function track(events) {
    ...
    var contract = findConfiguredContract();
    var normalized = events.map(function (event) {
      ...
      return {
        ...
        recommendation_id: Utility.trim(event.recommendation_id), campaign_id: 'LINE_PILOT',
        experiment_variant: 'P_GATE', asin: Utility.trim(event.asin), event_type: eventType,
        consent: true, source: 'LINE'
      };
    });
  ...
  function handleBridge(payload) {
    ...
    if (action === 'TRACK') { return track(payload.events); }
